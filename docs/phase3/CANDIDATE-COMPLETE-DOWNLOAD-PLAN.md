...
| `rpa_cv` | `is_referral`, `referred_by`, `referral_note`, `referral_set_by`, `referral_set_at` — plus `old_referred_by` / `new_referred_by` from the audit table, and the shape `/(^\|_)referr/i` | **Built 2026-09-04.** Sanghamitra, 23:33–26:08: *"none of the interview process should know that it is a, because then you can't be non-bias."* The dossier is the artefact that reaches an interviewer with no ATS account, so it is the hole that rule would otherwise have. Safe twice over: `CV_SELECT` is *derived from* `pickCvProfile()`, so these columns are never SELECTed, and `assertNoForbiddenFields()` refuses them if a future join drags one in. The pattern is **anchored** — an unanchored `/referr/i` also matches `PreferredShift`, a whitelisted profile field. See [REFERRAL-CANDIDATE-PLAN.md](../REFERRAL-CANDIDATE-PLAN.md). |
