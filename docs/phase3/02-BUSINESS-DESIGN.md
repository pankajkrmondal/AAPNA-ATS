...
7. **Closure** — final statuses: Approved, Rejected, On Hold, Candidate Withdrawn, Joined, Did Not Join, Joined and Left, Backed Out; ~~each triggers its communication~~. **Definitions confirmed by RT (Q12, 2026-07-13); the record auto-closes 90 days after Joined** unless marked Joined-and-Left first.
   > ⚠️ **Corrected 2026-08-26 — "each triggers its communication" is false for 5 of the 8.**
   > `SILENT_FINAL_OUTCOMES` (`stageNotification.service.js:87-93`) blocks **Joined, Joined and
   > Left, Backed Out, Did Not Join and Candidate Withdrawn** inside `resolveTemplate()` at
   > `:115` — *before* any template lookup, so an admin mapping cannot re-enable them. The
   > reasoning is sound and should stay: these five record something that already happened, and
   > there is nothing to tell the candidate they do not already know (emailing "Congratulations"
   > to someone who backed out is the failure this prevents). Only **Approved, Rejected and On
   > Hold** communicate, and they always do — `GENERIC_FALLBACK_BY_OUTCOME`
   > (`stageNotification.service.js:58-65`) covers all three with no mapping required. The
   > **vendor**, where there is one, is notified on every closure regardless
   > (`pipeline.service.js:1076`). See [PHASE3-CLOSURE-AUDIT-2026-08-26.md](../PHASE3-CLOSURE-AUDIT-2026-08-26.md) §2.8.
...
| **Client Interview (optional)** | **Mark-only (RT, 2026-08-25):** the round is arranged entirely offline and the app records the result — it is no longer bookable in-app. The recruiter transcribes the client's feedback into the outcome notes and sets Approve / Reject / Hold, which triggers candidate (+ vendor) communication. **The system never generates anything for the client** (Q14) — no invite, no reminder, and no interviewer scorecard, which previously went to whatever address was typed as the interviewer. Client contact details are still only the MRF's free-text `client_round_coordinator` / `client_details`; there is no per-journey client contact field. |
...
| **Offer** | **Record-only (Q3, RT 2026-07-13), reinforced 2026-07-14:** appointment/offer letters stay **entirely manual and outside the ATS** — HR prepares and shares them from their own mailbox/process. The ATS only needs the **stage/status to change**; it does not store the letter file at all. It records offer-shared date, candidate decision, and joining date. **⚠ Internal approval DISABLED (RT, 2026-08-25):** the recruiter approval step and its daily reminder nudge are switched off — the live flow is record shared → record decision → close. **This reverses the written answer to Q3 and the in-house Q26 decision and is owed a written confirmation from RT.** No validity timer; no offer version tracking (handled manually). Vendor receives at most a status-only notification, never the letter (Q5/Q29). |
| **Closure** | One-click final status ~~with automatic communication~~; definitions **confirmed** (Q12) — record auto-closes 90 days after Joined unless marked Joined-and-Left first. **⚠ Corrected 2026-08-26 (design intent vs. as-built):** (a) only Approved / Rejected / On Hold email the candidate — the other five are deliberately silent, see §2 rule 7; (b) **the UI reaches closure from the Offer stage only**, so a candidate who withdraws at Tech 2 cannot be closed at all, contradicting the Q12 definition of *Candidate Withdrawn* ("at **any** stage"). The API and templates already support any stage — only the button is missing. Both are logged with fixes in [PHASE3-CLOSURE-AUDIT-2026-08-26.md](../PHASE3-CLOSURE-AUDIT-2026-08-26.md) §2.1 / §2.8; **not yet built.** |
