...
| # | Item | Owner | Blocks |
|---|------|-------|--------|
| 1 | ~~Sample Evalground CSV~~ ✅ **Received 2026-07-14** — both bulk CSV (Naveen) and single-result Outlook report (Chhaya) shared in chat; **Harish still owes verification of the exact format** against the Section 1/2/3 design | Harish (verify) | M2 |
| 2 | ~~Harish's scorecard template~~ ✅ **Received 2026-07-14** ("Interview Evaluation Format V2.xlsx") — structure baked into M3 | — | closed |
| 3 | Bulk Evalground CSV: section→skill mapping approach not yet decided (single-candidate path can prompt at entry time; bulk cannot yet) | Harish | M2 import UI |
