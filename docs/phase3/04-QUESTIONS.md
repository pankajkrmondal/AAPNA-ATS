...
Companion docs: [01-PROCESS-UNDERSTANDING.md](01-PROCESS-UNDERSTANDING.md) · [02-BUSINESS-DESIGN.md](02-BUSINESS-DESIGN.md) · [03-DEVELOPMENT-PLAN.md](03-DEVELOPMENT-PLAN.md)
...
>
> **✅ Update (2026-07-15) — format verified, both A2 and A3 CLOSED.** Full write-up: [07-EVALGROUND-IMPORT-PLAN.md](07-EVALGROUND-IMPORT-PLAN.md). Summary:
> - **A2 (format verification):** the real 229-row bulk CSV and the Pradeep Gupta single-result report both confirm the Section 1/2/3 / email-key assumptions above. New, previously-unscoped findings: 6 extra Evalground skill-tag columns (`Sql, Coding, Python, Py Test, Playwright, Pywinauto`) not mentioned anywhere in docs 01-04; 6 of 229 rows have a blank email (malformed, not matchable); 5 rows have `Result=NA` (an abandoned/restarted attempt, distinct from a scored Fail); the single-result file is Word/MHTML with Outlook Safe-Links-wrapped report/resume URLs (not resolvable server-side). Full detail in doc 07 §1.
> - **A3 (section→skill mapping):** rejected manual RT tagging as too much work. Adopted instead: an AI step reads the test title (which already names the topics in section order, e.g. "General Aptitude, Python, SQL") to **suggest** a section→skill label + Aptitude/Technical bucket, cross-checked against the candidate's own skills and Evalground's own skill-tag columns as a corroboration signal, and **RT confirms once per import batch** (not per row) before commit — with a per-candidate correction option afterward if one candidate's classification looks wrong. Score rollup is computed generically per file (section max marks derived from the row data itself, never hardcoded), landing on the existing `rpa_cv.IQScore`/`TechScore` columns. Full detail in doc 07 §2. **Caveat:** this rule is validated against one sample test template only — recheck against a non-IT test (GA + English) and a pure role-specific test once more real exports are available.
...
|---|--------|-------------|------|
| Q1 | Email = key; >1 row/candidate possible; one file (GA+Tech); non-IT = GA+English, IT = role-specific by Role/MRF; via Evalground → Outlook. Two import mechanisms (bulk CSV + single via Outlook); retake overwrites only the score; Section 1/2/3 labels. **✅ 2026-07-15: format verified + bulk mapping approach decided (AI-suggested, RT-confirmed) — see [07-EVALGROUND-IMPORT-PLAN.md](07-EVALGROUND-IMPORT-PLAN.md)** | RT (annotated PDF + 2026-07-14 call); format/mapping by Harish | 2026-07-13 / 2026-07-14 / 2026-07-15 |
...
| # | Item | Owner | Blocks |
|---|------|-------|--------|
| 1 | ~~Sample Evalground CSV~~ ✅ **Received 2026-07-14, format verified 2026-07-15** — see [07-EVALGROUND-IMPORT-PLAN.md](07-EVALGROUND-IMPORT-PLAN.md) §1 | — | closed |
| 2 | ~~Harish's scorecard template~~ ✅ **Received 2026-07-14** ("Interview Evaluation Format V2.xlsx") — structure baked into M3 | — | closed |
| 3 | ~~Bulk Evalground CSV: section→skill mapping approach~~ ✅ **Decided 2026-07-15** — AI-suggested mapping (from the test title), RT confirms once per batch, correctable per candidate — see [07-EVALGROUND-IMPORT-PLAN.md](07-EVALGROUND-IMPORT-PLAN.md) §2 | — | closed |
