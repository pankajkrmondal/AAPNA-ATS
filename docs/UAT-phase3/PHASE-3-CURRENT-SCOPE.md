...
|---|---|---|
| 3.2 Sidebar — **DONE** | `MENU_ITEMS` — [MainLayout.jsx:77-98](../../frontend/src/layouts/MainLayout.jsx#L77-L98). Net change: `/mrf` moved from position 4 to position 2; every other entry keeps its relative order. **Vendor Dashboard needed no handling** — [MainLayout.jsx:253-254](../../frontend/src/layouts/MainLayout.jsx#L253-L254) locates it with `findIndex((m) => m.key === '/vendor')` and splices at `idx + 1`, so it self-positioned after the reorder. `BREADCRUMB_MAP` (lines 139-155) is keyed by path segment and is order-independent. `MENU_ITEMS` is not exported and is read only at [lines 250-256](../../frontend/src/layouts/MainLayout.jsx#L250-L256), so the blast radius is this one file. A comment above the array records that the order is HR-specified, so it does not get "tidied" later. | One file. No test suite asserts menu order. `vite build --mode staging` clean. |
| 3.2 — **what we deliberately did NOT touch** | An earlier draft of this annex said to mirror the new order onto `QUICK_ACTIONS` ([Dashboard.jsx:71-79](../../frontend/src/pages/Dashboard.jsx#L71-L79)) and `MODULES_INFO` ([AdminDashboard.jsx:63-73](../../frontend/src/pages/AdminDashboard.jsx#L63-L73)). **That instruction was wrong and has been dropped.** Neither list is a nav mirror: `QUICK_ACTIONS` is a curated 7-item shortcut set with different labels ("New MRF Request", "System Configuration") and no Pipeline / Vendor Dashboard / Email entries; `MODULES_INFO` is the **admin permission grid**, keyed by permission `key`, with its own audience. Reordering either would change an unrelated surface for no benefit to HR's request, which was explicitly about the left sidebar only. | Avoided — would have widened a zero-risk change into two unrelated screens during the freeze. |
